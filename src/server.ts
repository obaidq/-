...
type GameType = 'quiplash' | 'fibbage' | 'guesspionage' | 'fakinit' | 'triviamurder' | 'drawful';
...
function createPlayer(id: string, name: string, isHost: boolean, index: number): Player {
  return {
    id,
    name,
...
function getPlayerList(room: GameRoom): Player[] {
  return Array.from(room.players.values());
}

function countAnswered(room: GameRoom): number {
  return Array.from(room.players.values()).filter(p => p.currentAnswer !== null).length;
}

function allPlayersAnswered(room: GameRoom): boolean {
...
io.on('connection', (socket: Socket) => {
  ...
  // Start Game
  socket.on('startGame', ({ code, game }: { code: string; game: GameType }) => {
    const room = rooms.get(code);
  ...
  socket.on('submitAnswer', ({ code, answer }: { code: string; answer: any }) => {
    const room = rooms.get(code);
    if (!room) return;
  ...
  socket.on('submitVote', ({ code, voteId }: { code: string; voteId: string }) => {
    const room = rooms.get(code);
    if (!room) return;
  ...
  socket.on('requestNextRound', (code: string) => {
    const room = rooms.get(code);
    if (!room || socket.id !== room.hostId) return;
...
function startGameRound(room: GameRoom): void {
  room.players.forEach(p => p.currentAnswer = null);
...
function startQuiplashRound(room: GameRoom): void {
...
function startGuesspionageRound(room: GameRoom): void {
...
function startFakinItRound(room: GameRoom): void {
  const categories = Object.keys(CONTENT.fakinit) as Array<keyof typeof CONTENT.fakinit>;
...
function startTriviaMurderRound(room: GameRoom): void {
...
function startFibbageRound(room: GameRoom): void {
...
function startDrawfulRound(room: GameRoom): void {
  room.players.forEach((player, id) => {
...
function handleAllAnswered(room: GameRoom): void {
  switch (room.currentGame) {
    case 'quiplash':
...
function startVotingPhase(room: GameRoom): void {
...
function startFakinItVoting(room: GameRoom): void {
  room.gameData.votes = {};
...
function startFibbageVoting(room: GameRoom): void {
...
function calculateResults(room: GameRoom): void {
  switch (room.currentGame) {
    case 'quiplash':
...
function calculateQuiplashResults(room: GameRoom): void {
  const votes = room.gameData.votes || {};
  const voteCounts: Record<string, number> = {};
  
  room.gameData.matchup?.forEach((id: string) => {
    voteCounts[id] = 0;
  });
  ...
  const results = room.gameData.matchup?.map((id: string) => {
...
function calculateGuesspionageResults(room: GameRoom): void {
...
function calculateFakinItResults(room: GameRoom): void {
  const votes = room.gameData.votes || {};
...
function calculateTriviaMurderResults(room: GameRoom): void {
  const question = room.gameData.currentQuestion;
  const results: any[] = [];
...
function calculateFibbageResults(room: GameRoom): void {
